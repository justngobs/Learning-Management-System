	...
	<div id = "announcements">
		...
		<hr>
			...
			if(isset($_SESSION['current_subject'])){
				...
				$sql = "SELECT ass_id, sub_id, tea_id, ass_name, ass_description, date_posted FROM assignment WHERE sub_id = '$sub_id' ORDER BY ass_id DESC LIMIT 10";
				...
				if(mysqli_num_rows($result) > 0){
					...
					echo "<p style='margin-left:3%'>No assignments yet. Posted assignments will appear here.</p><br>";
				}
		...
		if(isset($_SESSION['current_subject'])){
			...
			$connection = mysqli_connect($host,$user,$password,$database) or die("<p>Error connecting to database: ".mysqli_error(). "</p>");
			$sub_id = $_SESSION['current_subject'];
			$sql = "SELECT quiz_id, sub_id, publish_date, due_date, quiz_title, quiz_description, status, n_of_questions FROM quiz WHERE sub_id ='$sub_id' AND status = '1' ";
			$result = mysqli_query($connection, $sql);
			...
			if(mysqli_num_rows($result) > 0){
				echo "
					...
					<hr>";
				while($row = mysqli_fetch_assoc($result)){
					echo "<h3 style='margin-left:2%'>".$row['quiz_title']."</h3>

						<p style='margin-left:4%'>".$row['quiz_description']."</p>

						<form method='post' action='quiz_assessment.php'>
							<input type='text' name='selected_quiz' value='".$row['quiz_id']."' hidden='hidden'>
							<input type='text' name='selected_quiz_title' value='".$row['quiz_title']."' hidden='hidden'>
							<input type='text' name='selected_quiz_description' value='".$row['quiz_description']."' hidden='hidden'>
							<input type='text' name='selected_quiz_n' value='".$row['n_of_questions']."' hidden='hidden'>
							<button style='color:white;background-color:green;border-radius:5px;margin-left:2%'>start quiz</button>
						</form><br>";
				}
				echo "</div>";
			}	
